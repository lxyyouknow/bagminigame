...
- 背包和备战区的多格物品显示需要同时支持两种皮肤方式：
  - 当前种菜皮肤可以按占格重复/拼接显示，例如小麦是 1x2 时上下显示两株。
  - 后续如果换成参考视频那种武器皮肤，同一个多格物品可能只用一张完整长图或横图，必须能按整体图片适配多格尺寸，不要把“多格一定多张图”写死。
...
如果界面已经出现严重结构性设计问题，例如层级错乱、布局模型不适合、多数量下不可读、需要新增组件形态或需要重做 HUD 表现，可以修改 `src/scenes/`、`src/windows/` 或新增 `src/ui/components/` 代码；不要硬靠 `s_ui_layout.json` 调参。
...
npm run test:battle-difficulty # 战斗难度、单波倍率、升级经验曲线
...
npm run test:battle-difficulty
...
- 除了必要的浏览器操作、读取控制台 log、复现报错或专项自测，不需要每次做完都再调用浏览器确认。
- 每轮修改完成后，优先完成命令行测试、JSON 校验和 `npm run build`，然后直接通知 lxy 让 lxy 亲自进浏览器测试手感。
- 如果修改涉及 Loading 卡住、首屏白屏、Canvas 不渲染、手机打不开、控制台运行时报错，再使用浏览器工具读取 log 和实际页面状态。
