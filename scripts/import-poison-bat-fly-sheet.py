...
SOURCE_DIR = ROOT / "public" / "game-assets" / "source" / "poison-bat-fly-down-video-20260623-112242"
OUT_DIR = ROOT / "public" / "game-assets" / "enemies" / "poison_bat_fly_down"
...
def keep_largest_alpha_component(image: Image.Image, threshold: int = 18) -> Image.Image:
    ...
    if not largest:
        raise RuntimeError("切帧后没有检测到毒蝠主体")
...
def write_outputs(frames: list[Image.Image]) -> None:
    ...
    for old in FRAME_DIR.glob("poison-bat-fly-down-*.png"):
        old.unlink()

    for index, frame in enumerate(frames):
        frame.save(FRAME_DIR / f"poison-bat-fly-down-{index:02d}.png", optimize=True)

    sheet = Image.new("RGBA", (FRAME_W * len(frames), FRAME_H), (0, 0, 0, 0))
    for index, frame in enumerate(frames):
        sheet.alpha_composite(frame, (index * FRAME_W, 0))
    sheet.save(OUT_DIR / "poison-bat-fly-down-sheet.png", optimize=True)

    frames[0].save(
        OUT_DIR / "poison-bat-fly-down.gif",
        save_all=True,
    ...
    preview.convert("RGB").save(OUT_DIR / "poison-bat-fly-down-contact-sheet.jpg", quality=92)

    metadata = {
        "name": "poison_bat_fly_down",
        ...
        "anchorY": 0.55,
        "scale": 0.45,
        "usage": "第二种小怪毒蝠，向屏幕下方快速飞行；由 lxy 提供的视频序列帧图集导入",
    }
