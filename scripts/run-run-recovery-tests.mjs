...
const outDir = ".tmp/run-recovery-tests";
...
await run("npx", [
  ...
  "tests/run-recovery-state.test.ts", "src/scenes/runRecoveryState.ts", "src/scenes/runSessionState.ts", "src/types.ts",
]);
await run("node", [`${outDir}/tests/run-recovery-state.test.js`]);
