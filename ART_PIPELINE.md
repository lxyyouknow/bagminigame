...
- 弹道自身动画和移动速度必须分开：`s_animation.json.fps` 只控制旋转/闪烁节奏，`s_skill.json.speed` 控制每秒移动像素。
- 单张旋转武器可以只交付一张透明 PNG，再由脚本按固定角度生成循环序列帧，不需要美术重复画 8 张。
- 爆炸、烟雾、液体飞溅可能跨格，不能默认按 sheet 宽度等分；应按连通区域和帧中心切分，并检查 contact sheet。
- 所有帧必须使用相同画布、锚点和缩放基准。消散尾帧不能单独放大。
- 当前番茄爆炸使用 `256x256 / 8 帧 / 18 FPS / loop=false / scale=0.5`。
- 命中特效首帧主体较完整时，缩放过大会看起来像目标点突然生成一个巨大武器；调整 `s_animation.json.scale`，不要重新缩放每张 PNG。
- `projectileAnimKey` 和 `hitAnimKey` 是当前统一入口。新增专属武器表现时，不要用另一条代码分支抢先 return，导致通用命中特效失效。
- 开战时武器先走完整 CD，并少量错峰；目标选择会优先避开已有大量在途弹道的怪物。
- 飞行物命中、目标死亡或场景销毁时立即释放；非循环爆炸在播放完成后释放。共享纹理继续由资源管理器缓存。

常见异常定位：

- 有番茄但没爆炸：检查命中分支是否提前 `return`，再检查 `hitAnimKey` 和 `loop=false`。
- 爆炸出现两个半圆：检查源 sheet 是否跨格，重新运行连通区域切帧脚本。
- 第一只怪出现时满屏弹道：检查进入战斗时是否重置成完整 CD，以及目标分流是否统计在途弹道。
- 爆炸太大：优先调整 `s_animation.json` 的 `scale`，番茄当前命中缩放为 `0.5`。
